...
internal static class EnemyAIPatches {
    ...
    [HarmonyPostfix]
    private static void BlobModifications(BlobAI __instance) {
    ...
    [HarmonyPostfix]
    private static void FlowerManModifications(FlowermanAI __instance) {
        if (__instance.isOutside) {
            if (__instance.OwnerClientId != GameNetworkManager.Instance.localPlayerController.actualClientId) {
                __instance.ChangeOwnershipOfEnemy(GameNetworkManager.Instance.localPlayerController.actualClientId);
            }
    ...
    [HarmonyPostfix]
    private static void HoarderBugModifications(HoarderBugAI __instance) {
        if (__instance.isOutside) {
            if (__instance.OwnerClientId != GameNetworkManager.Instance.localPlayerController.actualClientId) {
                __instance.ChangeOwnershipOfEnemy(GameNetworkManager.Instance.localPlayerController.actualClientId);
            }
    ...
    private static bool HoldState;

    [HarmonyPatch(typeof(JesterAI), "Update")]
    [HarmonyPostfix]
    private static void InsideOutJesterModifications(JesterAI __instance) {
    ...
    [HarmonyPostfix]
    private static void InsideOutSpringManModifications(SpringManAI __instance) {
        if (__instance.isOutside) {
            if (__instance.OwnerClientId != GameNetworkManager.Instance.localPlayerController.actualClientId) {
                __instance.ChangeOwnershipOfEnemy(GameNetworkManager.Instance.localPlayerController.actualClientId);
            }
    ...
    [HarmonyPrefix]
    private static bool InsideOutStartSearchModification(EnemyAI __instance, ref Vector3 startOfSearch,
        ...
        if (Variables.InsideOutOwnership && RoundManager.Instance.IsHost) {
            if (Variables.SpawnedInsideOutID.Contains(__instance.GetInstanceID())) {
                ...
                Plugin.Logger.LogError("Changed Search Start Location of Enemy insideOut");
                return true;
            }
        }
    ...
    [HarmonyPatch(typeof(JesterAI), "Update")]
    [HarmonyPostfix]
    private static void InstaJesterModifications(JesterAI __instance) {
    ...
    [HarmonyPostfix]
    private static void RumblingForestGiantModifications(ForestGiantAI __instance, bool ___inEatingPlayerAnimation) {
    ...
    //[HarmonyPrefix]
    //private static bool ChangeOwnershipOfEnemyPatch(EnemyAI __instance)
    //{
    ...
    [HarmonyPostfix]
    private static void InsideMouthDogModifications(MouthDogAI __instance) {
    ...
    [HarmonyPrefix]
    private static bool MouthDogAIOnCollideWithPlayerPatch(MouthDogAI __instance, ref Collider other,
        bool ___inKillAnimation, Collider ___debugCollider, bool ___inLunge, Ray ___ray, RaycastHit ___rayHit,
        RoundManager ___roundManager) {
        if (Variables.DogForceOwnership && RoundManager.Instance.IsHost) {
            ...
            if (target != null && !target.isPlayerDead && target.isPlayerControlled && !___inKillAnimation) {
                var a = Vector3.Normalize((__instance.transform.position + Vector3.up -
                                           target.gameplayCamera.transform.position) * 100f);
                RaycastHit raycastHit;
                if (Physics.Linecast(__instance.transform.position + Vector3.up + a * 0.5f,
    ...
    [HarmonyPostfix]
    private static void FlowerManAIOnCollideWithPlayerPatch(FlowermanAI __instance, ref Collider other,
        bool ___startingKillAnimationLocalClient) {
        if (true) {
            var target = other.gameObject.GetComponent<PlayerControllerB>();
    ...
    [HarmonyPostfix]
    private static void JesterAIOnCollideWithPlayerPatch(JesterAI __instance, ref Collider other,
    ...
    [HarmonyPostfix]
    private static void BlobAIOnCollideWithPlayerPatch(BlobAI __instance, ref Collider other) {
        if (true) {
            var target = other.gameObject.GetComponent<PlayerControllerB>();

            if (target != null && !target.isPlayerDead && target.isPlayerControlled) {
                var blobinfo = typeof(BlobAI);
                var timeSinceHittingLocalPlayer = blobinfo.GetField("timeSinceHittingLocalPlayer",
                    BindingFlags.NonPublic | BindingFlags.Instance);
                var tamedTimer = blobinfo.GetField("tamedTimer", BindingFlags.NonPublic | BindingFlags.Instance);
                var angeredTimer =
                    blobinfo.GetField("angeredTimer", BindingFlags.NonPublic | BindingFlags.Instance);
    ...
    [HarmonyPostfix]
    private static void HoarderBugAIOnCollideWithPlayerPatch(HoarderBugAI __instance, ref Collider other) {
        var target = other.gameObject.GetComponent<PlayerControllerB>();
        if (target != null && !target.isPlayerDead && target.isPlayerControlled) {
            if (!__instance.inChase || __instance.timeSinceHittingPlayer < 0.5f) return;
